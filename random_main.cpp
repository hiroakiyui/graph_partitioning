...
289,1089
     ...
     cout << "random criteria: " <<end_time- begin_time << endl;
     delete p_graph;
 }
 Graph* Graph::Insert(){
     vector<int> rnmvtx;
     rnmvtx.reserve(V);
     for(int i=0; i < V; i++) rnmvtx.push_back(i);     
     int mtrand;
     init_genrand((unsigned)time(NULL));
     vector<int>::iterator irp;

     for(irp=rnmvtx.begin(); irp!=rnmvtx.end(); ++irp){ //shufle by Fisher-Yates
    ...
    vector<int>::iterator irm;
   
    int weightCount=1;
    for(irm=rnmvtx.begin(); irm!=rnmvtx.end(); irm++){
         for(itr=adj_list_edge[*irm].begin(); itr !=adj_list_edge[*irm].end(); ++itr){
             if(*irm!=*itr){
                     graph->edge[weightCount].src =*irm;
                     graph->edge[weightCount].dest = *itr;
                     graph->edge[weightCount].weight = weightCount;
                     ++weightCount;  
             }         
         }
    } 
