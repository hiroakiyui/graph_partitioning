 ...
 class Graph {  
  private:
   ...
   public:
      ...
      vector< vector<int> > Partitioning(vector<int> longestpath, Graph G);
...
void Graph::EdgeWeight(){
    //const clock_t weight_begin=clock(); 
    vector<bool> cluster(E-V, false);
    vector<int>::iterator adj_vtx; 
    int weight;   
    int vertex_weight;
    for(int i=0; i<V; ++i){   
        vertex_weight=adj[i].size();
         for(adj_vtx=adj_list_edge[i].begin(); adj_vtx!=adj_list_edge[i].end(); ++adj_vtx){            
             if(i!=*adj_vtx){ // no include diagonal elements 
                 weight=vertex_weight+adj[*adj_vtx].size(); 
                 if(!cluster[weight])cluster[weight]=true;           
                 wgtvtx[weight].push_back(*adj_vtx);
                 wgtadj[weight].push_back(i);
             }   
         }
    }
   for(int i=0; i<E-V; ++i)if(cluster[i]==true)bucket.push_back(i);

   init_genrand((unsigned)time(NULL));
   int rnd;
   vector<int>::iterator irp;
   for(irp=bucket.begin(); irp!=bucket.end(); ++irp){ //shufle by Fisher-Yates
       //rnd =genrand_int31()%(wgtvtx[*irp].size());
       for(int p_weight=wgtvtx[*irp].size()-1; p_weight > 0; p_weight--){  
              rnd =genrand_int31()%(wgtvtx[*irp].size());           
              swap(wgtvtx[*irp][p_weight],wgtvtx[*irp][rnd]);
              swap(wgtadj[*irp][p_weight],wgtadj[*irp][rnd]);
       }     
   }  
  //const clock_t weight_end=clock();
  //cout << "Weight : " << float(weight_end-weight_begin )/CLOCKS_PER_SEC << endl;
 }
 const int Graph::NUMBER_OF_VERTICES(5489);
 void Graph::graph_read(){
     const char *graph_name="s3rmt3m1.txt";
     Graph vtx_edge(
5489,111579
);
     ...
     cout << "minimum criteria: " <<end_time- begin_time << endl;
     delete p_graph;
 }
 Graph* Graph::Insert(){
      ...
      vector<int>::iterator brt;
      int weightCount=1;
      int wgtvtx_size;
      for(brt=bucket.begin(); brt!=bucket.end(); ++brt){      
...
vector<int> Graph::FindLongestPath(){   
    ...
    while(!queue.empty()){
        ...
        queue.pop_front();
        for(vtx2=mst[far_vtx].begin(); vtx2!=mst[far_vtx].end(); ++vtx2){
            if(!visited[*vtx2]){                
                visited[*vtx2]=true;
  ...
  int sum=0;
  for(itx=adj_size.begin(); itx!=adj_size.end(); ++itx){     
        sum=sum+(*itx);
...
vector< vector<int> > Graph::Partitioning(vector<int> longestpath, Graph G){
    ...
    for(int p=1; p<longestpath_size; ++p){
          ...
          //for(Uvtx=U.begin(); Uvtx!=U.end(); ++Uvtx)bvU.set(*Uvtx,1);
          for(Uvtx=U.begin(); Uvtx!=U.end(); ++Uvtx){                   
              bvU.set(*Uvtx,1);
              for(AdjU=G.adj[*Uvtx].begin();AdjU!=G.adj[*Uvtx].end();++AdjU){
                        bvAdjU.set(*AdjU,1);//Adj(U)
              } 
          }  
          bvNewseparator=(bvAdjU & ~bvU)|(bvSeparator & ~bvU);
